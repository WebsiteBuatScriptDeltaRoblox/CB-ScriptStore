...
async function openPublicProfile(userId){
  ...
  const avatar=$('#publicProfileAvatar'),name=$('#publicProfileName'),scripts=$('#publicProfileScripts');
...
async function openPublishModal(){
  ...
  box.innerHTML=rows.map(s=>{const isPublic=s.visibility==='public';return `<div class="card" style="margin-bottom:12px"><div class="card-head"><div class="icon">&lt;/&gt;</div><div><h3>${escapeHtml(s.filename)}</h3><small>${escapeHtml(visibilityLabel(s.visibility))}</small></div></div><div class="card-foot"><span class="tag">${escapeHtml(visibilityLabel(s.visibility))}</span><button class="mini-btn ${isPublic?'active':''}" data-toggle-public="${s.id}" data-next-visibility="${isPublic?'private':'public'}">${escapeHtml(isPublic?tr('unpublish'):tr('publishNow'))}</button></div></div>`;}).join('');
  box.querySelectorAll('[data-toggle-public]').forEach(b=>b.onclick=()=>togglePublication(b.dataset.togglePublic,b.dataset.nextVisibility));
}
async function togglePublication(id,nextVisibility){
  await syncAuth(); if(!currentUser)return;
  const err=$('#publishError'); if(err){err.textContent='';err.style.color='';}
  try{
    const {error}=await sb.from('scripts').update({visibility:nextVisibility,updated_at:new Date().toISOString()}).eq('id',id).eq('user_id',currentUser.id);
    if(error)throw error;
    if(err){err.style.color='#28d9a4';err.textContent=nextVisibility==='public'?(tr('published')||'Script berhasil dibuat Public.'):(tr('unpublished')||'Publikasi dibatalkan.');}
    await renderPublicScripts();
    await renderMyPublicScripts();
    await renderScriptsIfPossible();
    await openPublishModal();
  }catch(e){if(err){err.style.color='#ff7690';err.textContent=e.message||'Gagal mengubah status publikasi.';}}
}
...
async function syncAuth(){
  ...
  if(currentUser) await ensureProfileRecord();
  return session;
}
...
const commonExtra = {myScriptsButton:'My Scripts',scriptCreator:'Script creator',websiteLink:'Website link',viewScript:'Script',copyRaw:'Copy Raw Link',copyScript:'Copy Script',copiedScript:'Copied',navWorkspace:'Script',workspaceTitle:'Script',workspaceDesc:'View and manage your saved scripts.',createScript:'Create Script',createDesc:'Write your Luau script, then save it to your account.',profile:'Profile',profileGuest:'Log in to view your profile.',notLoggedIn:'Not logged in',loggedIn:'Logged in',logout:'Logout',scriptFilename:'Script filename',private:'Private',public:'Public',saveScript:'Save Script',newScript:'New Script',savedScripts:'Saved Scripts',newScriptName:'New script',saved:'Script saved.',updated:'Script updated.',deleted:'Script deleted.',confirmDelete:'Delete this script?',needLoginWorkspace:'Log in to use your script workspace.',raw:'Raw →',edit:'Edit',delete:'Delete',unpublish:'Cancel publication',unpublished:'Publication canceled.',navChat:'Global Chat',chatTitle:'Global Chat',chatDesc:'Talk with other CB ScriptStore users in real time.',onlineNow:'Online Now',chatLogin:'Register or login to join the global chat.',chatPlaceholder:'Write a message...',send:'Send',noMessages:'No messages yet. Start the conversation!',chatError:'Chat could not be loaded.'};
...
async function saveScript(e){
  ...
  const id=$('#scriptId').value, rawFilename=$('#scriptFilename').value.trim(), filename=sanitizeScriptFilename(rawFilename), code=$('#scriptCode').value, visibility=id?$('#scriptVisibility').value:'private';
  if(!rawFilename || filename.length<1 || filename.length>120){err.textContent='Nama script tidak valid.';return;}
  if(code.length>500000){err.textContent='Script terlalu panjang.';return;}
  try{
    if(filename!==rawFilename){err.style.color='#f0c84b';err.textContent=currentLang()==='id'?'Nama script mengandung kata terlarang dan diubah menjadi the Sensor.':'The script name contained a blocked word and was changed to the Sensor.';}
...
function resetScriptForm(clearMessage=true){$('#scriptId').value='';$('#scriptFilename').value='';$('#scriptCode').value='';$('#scriptVisibility').value='private';const pubOpt=$('#scriptVisibility option[value=public]');if(pubOpt)pubOpt.disabled=true;if(clearMessage){$('#scriptError').textContent='';$('#scriptError').style.color='';}}
function editScript(s){if(!s)return;$('#scriptId').value=s.id;$('#scriptFilename').value=s.filename;$('#scriptCode').value=s.code;const pubOpt=$('#scriptVisibility option[value=public]');if(pubOpt)pubOpt.disabled=false;$('#scriptVisibility').value=s.visibility;window.location.hash='#create';}
