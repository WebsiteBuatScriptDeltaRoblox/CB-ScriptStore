...
ui.id.myScriptsButton='Lihat Script Saya'; ui.id.scriptCreator='Pembuat script'; ui.id.websiteLink='Link website'; ui.id.viewScript='Script'; ui.id.navWorkspace='Script'; ui.id.workspaceTitle='Script'; ui.id.workspaceDesc='Lihat dan kelola script yang sudah kamu simpan.'; ui.id.createScript='Buat Script'; ui.id.createDesc='Tulis script Luau kamu, lalu simpan ke akunmu.'; ui.id.profile='Profil'; ui.id.profileGuest='Login untuk melihat profil.'; ui.id.notLoggedIn='Belum login'; ui.id.loggedIn='Sudah login';
ui.id.myScriptsButton='Lihat Script Saya'; ui.id.scriptCreator='Pembuat script'; ui.id.websiteLink='Link website'; ui.id.viewScript='Script'; ui.id.navWorkspace='Script'; ui.id.workspaceTitle='Script'; ui.id.workspaceDesc='Lihat dan kelola script yang sudah kamu simpan.'; ui.id.logout='Logout'; ui.id.scriptFilename='Nama file script'; ui.id.private='Private'; ui.id.public='Public'; ui.id.saveScript='Simpan Script'; ui.id.newScript='Script Baru'; ui.id.savedScripts='Script Tersimpan'; ui.id.newScriptName='script-baru.lua'; ui.id.saved='Script berhasil disimpan.'; ui.id.updated='Script berhasil diperbarui.'; ui.id.deleted='Script berhasil dihapus.'; ui.id.confirmDelete='Hapus script ini?'; ui.id.needLoginWorkspace='Login untuk menggunakan workspace script kamu.'; ui.id.raw='Raw →'; ui.id.edit='Edit'; ui.id.delete='Hapus';
...
async function renderMyPublicScripts(){
  ...
  try{
    ...
    box.innerHTML=rows.map(s=>`<article class="public-card" data-select-public="${s.id}" tabindex="0" role="button"><h3>${escapeHtml(s.filename)}</h3><div class="public-meta">${escapeHtml(tr('scriptCreator'))}: @${escapeHtml(currentUser?.user_metadata?.username||'User')}</div><div class="public-meta">${escapeHtml(tr('websiteLink'))}: ${escapeHtml(SITE_URL)}</div><div class="public-meta">👁 ${Number(s.view_count||0)}</div><div class="script-actions"><a class="raw mini-btn" href="${rawUrl(s.id)}" target="_blank" rel="noreferrer">${escapeHtml(tr('viewScript'))}</a><button class="mini-btn" data-select-public-btn="${s.id}">${escapeHtml(tr('selectScript'))}</button><button class="mini-btn" data-copy-my-public="${s.id}">${escapeHtml(tr('copyRaw'))}</button></div></article>`).join('');
    box.querySelectorAll('[data-select-public], [data-select-public-btn]').forEach(el=>el.addEventListener('click',e=>{ if(e.target.closest('a'))return; const id=el.dataset.selectPublic||el.dataset.selectPublicBtn; if(id) window.open(rawUrl(id),'_blank','noopener,noreferrer'); }));
    box.querySelectorAll('[data-copy-my-public]').forEach(b=>b.onclick=e=>{e.stopPropagation();navigator.clipboard?.writeText(rawUrl(b.dataset.copyMyPublic));});
